...
class iOSGenerator(BaseGenerator):
    ...
    def process(self):
        ...
        if self.__class__ == ObjectiveCGenerator:
            progress('YESSSSPreparing to generate Objective-C templates')
            self.check_reserved_words(self.schema.data_objects)
            print('BOBOB')
...
class ObjectiveCGenerator(iOSGenerator):
	...
	def check_reserved_words(self, data_objects):
		progress(data_objects)
    pass
